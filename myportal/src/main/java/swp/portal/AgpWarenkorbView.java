...
public class AgpWarenkorbView extends PolymerTemplate<AgpWarenkorbView.AgpWarenkorbViewModel>
	...
	public AgpWarenkorbView() {
		...
		buttonRemoveArtikel.addClickListener(e -> {
			vaadinGrid.getSelectionModel().getFirstSelectedItem()
					.ifPresent(item -> userMB.removeFromWarenkorb(item.getGeraeteID()));
			fillGrid();
		});
		...
		buttonKaufen.addClickListener(e -> {
			...
			for (GeraetTO gTO : userMB.getWarenkorb()) {
				warenkorb.put(gTO.getGeraeteID(),
						warenkorb.get(gTO.getGeraeteID()) == null ? 1 : warenkorb.get(gTO.getGeraeteID()) + 1);
			}

			for (Map.Entry<Integer, Integer> entry : warenkorb.entrySet()) {
				if (geraetMB.getGeraet(entry.getKey()).getAnzahl() < entry.getValue()) {
					Notification.show("Artikel nicht ausreichend verfügbar: "
							+ geraetMB.getGeraet(entry.getKey()).getGeraetename());
					return;
				}
			}

			systemMB.createRechnung(userMB.getMatrikelNr(), userMB.getName(), userMB.getSurname(), userMB.getEmail(),
					userMB.getWarenkorb(), warenkorb);
	...
	private void prepare() {
		...
		vaadinGrid.addColumn(new NumberRenderer<>(GeraetTO::getPreis, "€ %(,.2f", Locale.GERMAN, "€ 0.00"))
				.setHeader("Preis").setTextAlign(ColumnTextAlign.END).setSortable(true);
	}
